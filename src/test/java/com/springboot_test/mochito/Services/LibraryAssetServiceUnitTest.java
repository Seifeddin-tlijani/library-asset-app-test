...
public class LibraryAssetServiceUnitTest {
    ...
    public void whenAssetsQueried_thenDetailsAsExpected(String queriedBy, String queryValue) {
        ...
        if ("Title".equalsIgnoreCase(queriedBy)) {
            Mockito.when(this.mockRepository.findByAssetTitle(queryValue))
                    .thenReturn(this.baseAssets);

            objAssets = this.assetService.findByTitle(queryValue);
        }

        Assertions.assertEquals(this.baseAssets.size(), objAssets.size(), "Asset size mismatch");
        for (int idx = 0; idx < this.baseAssets.size(); idx++) {
            Assertions.assertEquals(this.baseAssets.get(idx).getAssetTitle(), objAssets.get(idx).getAssetTitle(),
                    "Asset title mismatch at index " + idx);
        }
    }

    private static Stream<Arguments> provideQueryData() {
        return Stream.of(
                Arguments.of("Title", "Things Fall Apart"),
                Arguments.of("Title", "Trans Atlantic Jazz")
        );
    }
